...
3. **Create a ZIP file**:
   ...
   - Name it: `ATS_WPF_Portable_v1.0.zip` (or your version)
...
4. **Recommended: Include Documentation**
   ...
   - Add `README.md` to the ZIP (project overview)
...
5. Confirm that `ATS_TwoWheeler_Updater.exe` exists next to `ATS_WPF.exe`
...
3. Choose where to extract (Desktop is fine, or create a folder like `C:\ATS_TwoWheeler\`)
4. Click **Extract**
...
2. Run `ATS_WPF.exe` directly from the USB
...
- [ ] README.md or USER_GUIDE.md included (optional but helpful)
